...
namespace Server
{
    public partial class Server : Form
    {
        ...
        private void Connection(MyClient obj)
        {
            ...
            AddToGrid(obj.id, obj.name);
            string msg = string.Format("SYSTEM: {0} has connected", obj.name);
            Log(msg);
            Send(msg, obj.id);
            ...
            msg = string.Format("SYSTEM: {0} has disconnected", tmp.name);
            Log(msg);
            Send(msg, tmp.id);
        }

        private void Listener(IPAddress addr, int port)
        {
                ...
                while (active)
                {
                    if (listener.Pending())
                    {
                        try
                        {
                            ...
                            obj.id = id;
                            obj.name = string.Format("Client {0}", obj.id);
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                {
                    listener = new Thread(() => Listener(addr, port))
                    {
                        IsBackground = true
                    };
                    listener.Start();
        ...
        private void BeginWrite(string msg, long id = -1) // send the message to everyone if ID is not supplied or is -1
        {
            byte[] buffer = Encoding.UTF8.GetBytes(msg);
            foreach (KeyValuePair<long, MyClient> obj in clients)
            {
                if (id != obj.Value.id && obj.Value.client.Connected) // do not send the message back to the sender
                {
                    try
                    {
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    sendTextBox.Clear();
                    Log(string.Format("{0} (You): {1}", nameTextBox.Text, msg));
                    Send(string.Format("{0}: {1}", nameTextBox.Text, msg));
                }
            }
        }
